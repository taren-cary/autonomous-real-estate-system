...
### From CEO Agent (extracted from the signed contract)
```
CLIENT_NAME   — client's display name (e.g. "Sarah Johnson")
CLIENT_EMAIL  — client's email address
```

### Generated by the Fulfillment Agent (not required as inputs)
```
CLIENT_ID     — auto-generated slug from CLIENT_NAME (e.g. "sarah-johnson-realty")
VPS_HOST      — obtained from Hostinger API after programmatic VPS purchase (Phase 0)
VPS_USER      — defaults to "root"
VOICE_ID      — selected in Phase 2b (defaults to 11labs-cleo)
CLIENT_PHONE  — provisioned in Phase 2c via Twilio API
```

### From Client Intake Form (client fills this in after receiving the form link)
```
CLIENT_GMAIL              — client's Gmail address (e.g. "sarah@sarahjohnsonrealty.com")
CLIENT_TIMEZONE           — IANA timezone (e.g. "America/New_York")
CLIENT_STATE              — US state abbreviation (e.g. "MD", "CA", "TX")
CLIENT_GHL_LOCATION_ID    — GHL sub-account ID (from GHL URL: app.gohighlevel.com/v2/location/{ID}/...)
CLIENT_LISTING_SHEET_ID   — Google Sheet ID (from sheet URL: docs.google.com/spreadsheets/d/{ID}/edit)
ANTHROPIC_API_KEY         — client's own Anthropic API key (from console.anthropic.com/settings/keys)
CLIENT_TELEGRAM_BOT_TOKEN — Telegram bot token. Client instructions:
...
Do not start Phase 1 until all form inputs are received and stored in the client_intake table.
