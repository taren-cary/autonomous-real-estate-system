...
All inputs are read from the `client_intake` table in Supabase — do not ask the CEO Agent for values verbally. Do not start Phase 1 until the `client_intake` row is fully populated.

### Sales Agent populates (extracted from the signed contract)
```
CLIENT_NAME     — real estate agent's personal name (e.g. "Sarah Johnson")
CLIENT_COMPANY  — the real estate firm/brokerage name (e.g. "Johnson Realty Group")
CLIENT_EMAIL    — agent's email address
```

### Fulfillment Agent generates (do not ask for these — create or derive them)
```
CLIENT_ID   — slug from CLIENT_NAME, lowercase hyphens only (e.g. "sarah-johnson-realty")
VPS_HOST    — written to client_intake by the purchase-vps skill after Hostinger provisions the VPS
VPS_USER    — defaults to "root"
```
Note: `VOICE_ID` is set in Phase 2b. `CLIENT_PHONE` (stored as `twilio_number` in the clients table) is provisioned in Phase 2c.

### Client fills in via intake form
```
CLIENT_GMAIL              — business Gmail (e.g. "sarah@sarahjohnsonrealty.com")
CLIENT_TIMEZONE           — IANA timezone (e.g. "America/New_York")
CLIENT_STATE              — US state abbreviation (e.g. "MD", "CA", "TX")
CLIENT_GHL_LOCATION_ID    — GHL sub-account ID (app.gohighlevel.com/v2/location/{ID}/...)
CLIENT_LISTING_SHEET_ID   — Google Sheet ID (docs.google.com/spreadsheets/d/{ID}/edit)
ANTHROPIC_API_KEY         — client's Anthropic API key (console.anthropic.com/settings/keys)
CLIENT_TELEGRAM_BOT_TOKEN — Telegram bot token:
                            1. Open Telegram → search @BotFather → send /newbot
                            2. Choose a name (e.g. "Johnson Realty Assistant")
...
LLM_RESP=$(curl -s -X POST "https://api.retellai.com/create-retell-llm" \
  ...
  -d "{
    ...
    \"general_prompt\": \"You are an outbound intake agent calling leads on behalf of {{agent_name}} from {{company_name}}. The lead expressed interest in {{property_interest}} from {{lead_source}}. Qualify them by asking these 6 questions one at a time — never ask more than one per turn: (1) Are you looking to buy or sell? (2) What is your timeline — how soon are you looking to make a move? (3) What is your budget range? (4) Have you been pre-approved for financing? (5) What areas or neighborhoods are you most interested in? (6) Are you currently working with another agent? After all 6 answers are collected, call score_lead. If routing is book_showing, call check_availability then book_showing. Confirm the booking verbally with the lead before ending the call. When a lead asks about available properties or listings, call get_listings with relevant filters from their stated preferences. Always pass agent_id as $CLIENT_ID in every tool call. Never mention being an AI unless directly asked.\",
    \"general_tools\": [
      {
        \"type\": \"custom\",
        \"name\": \"score_lead\",
...
You are the first point of contact for every lead coming in via SMS, email, or phone for $CLIENT_COMPANY.
...
### Step 4i — Write Complete openclaw.json
...
GATEWAY_TOKEN=$(grep CLAWDBOT_GATEWAY_TOKEN ~/.clawdbot/.env | cut -d'=' -f2-)

# Write the complete openclaw.json — one shot, all settings included
cat > $HOME/.openclaw/openclaw.json << EOF
{
  ...
  "agents": {
    ...
    "list": [
      {
        "id": "intake-agent-$CLIENT_ID",
        "workspace": "$HOME/.openclaw/$CLIENT_ID/workspace-intake",
        "model": { "primary": "anthropic/claude-sonnet-4-6" },
        "skills": ["calendar-check", "calendar-book", "crm-write", "crm-read", "gmail-send", "email-check", "score-lead", "get-listings", "sms-send"]
      },
      {
        "id": "showing-coordinator-$CLIENT_ID",
        "workspace": "$HOME/.openclaw/$CLIENT_ID/workspace-showing-coordinator",
        "model": { "primary": "anthropic/claude-haiku-4-5-20251001" },
        "heartbeat": { "every": "30m" },
        "skills": ["calendar-check", "calendar-book", "crm-write", "crm-read", "gmail-send", "sms-send"]
      },
      {
        "id": "deadline-monitor-$CLIENT_ID",
        "workspace": "$HOME/.openclaw/$CLIENT_ID/workspace-deadline-monitor",
        "model": { "primary": "anthropic/claude-haiku-4-5-20251001" },
        "heartbeat": { "every": "1h" },
        "skills": ["crm-write", "crm-read", "gmail-send"]
      },
      {
        "id": "listings-market-$CLIENT_ID",
        "workspace": "$HOME/.openclaw/$CLIENT_ID/workspace-listings-market",
        "model": { "primary": "anthropic/claude-sonnet-4-6" },
        "skills": ["crm-write", "crm-read", "gmail-send", "get-listings", "zillow", "listing-writer", "cma-research", "mls-monitor"]
      },
      {
        "id": "admin-agent-$CLIENT_ID",
        "workspace": "$HOME/.openclaw/$CLIENT_ID/workspace-admin",
...
# Validate the written JSON before continuing
python3 -m json.tool $HOME/.openclaw/openclaw.json > /dev/null \
  && echo "✓ openclaw.json valid" \
  || { echo "✗ Invalid JSON — do not restart gateway"; exit 1; }
```
